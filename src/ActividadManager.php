...
class TemaManager implements IDWESEntidadManager{
  ...
  public static function obtenerActividadPorIdParticipante($id){
    $db = DWESBaseDatos::obtenerInstancia();
    $db -> ejecuta("SELECT a.id, a.descripcion, a.fecha, a.n_participantes, a.lugar FROM actividad a WHERE id = (SELECT p.actividad_id FROM participa p WHERE usuario_id = ? )");
